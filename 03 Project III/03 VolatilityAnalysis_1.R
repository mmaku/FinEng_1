...
sigma_X<-0.196
sigma_Y<-0.263
w<-exp(-T*(r_PLN-r_f+rho*sigma_Y*sigma_X))
V0<-100*w
...
deltaVrPLN<-100*exp((-1)*T*(r_PLN_wek-r_f+rho*sigma_Y*sigma_X))-V0
...
write.csv(cbind(r_PLN_proc,VrPLNproc),paste(MyDeliverables,"1_rPLN",sep=""))
...
deltasigmaX<-100*exp((-1)*T*(r_PLN-r_f+rho*sigma_Y*sigma_X_wek))-V0
...
write.csv(cbind(sigma_X_proc,VsigmaXproc),paste(MyDeliverables,"1_sigma_X",sep=""))
...
deltasigmaY<-100*exp((-1)*T*(r_PLN-r_f+rho*sigma_Y_wek*sigma_X))-V0
...
write.csv(cbind(sigma_Y_proc,VsigmaYproc),paste(MyDeliverables,"1_sigma_Y",sep=""))
...
deltarho<-100*exp((-1)*T*(r_PLN-r_f+rho_wek*sigma_Y*sigma_X))-V0
...
write.csv(cbind(rho_proc,Vrhoproc),paste(MyDeliverables,"1_rho",sep=""))
...
for(i in 1:100)
{
    for(j in 1:100)
    {
        macierz[i,j] <- (100)*exp(-(r_PLN-r_f+rho*sigmaY_seq[j]*sigmaX_seq[i]))
    }
}
write.csv(macierz,paste(MyDeliverables,"1_MACsigma",sep=""))

plot <- plot_ly(z = macierz, x = sigmaY_seq, y = sigmaX_seq, type = "surface")
plotly_POST(plot, filename = "1_MACsigma", world_readable=TRUE)
